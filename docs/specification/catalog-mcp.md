...
| :--- | :--- | :--- |
| `search_catalog` | [Search Catalog](catalog.md#search-catalog) | Search for products. |
| `get_catalog_item` | [Get Catalog Item](catalog.md#get-catalog-item) | Get a product or variant by ID. |

### `search_catalog`

Maps to the [Search Catalog](catalog.md#search-catalog) operation.

{{ method_fields('search_catalog', 'rest.openapi.json', 'catalog-mcp') }}
...
{{ method_fields('get_catalog_item', 'rest.openapi.json', 'catalog-mcp') }}
...
envelope and optional `messages` array. See [Catalog Capability](catalog.md#messages-and-error-handling)
...
2. Provide both `search_catalog` and `get_catalog_item` tools.
3. Require `query` parameter for `search_catalog`.
4. Require `id` parameter for `get_catalog_item`.
